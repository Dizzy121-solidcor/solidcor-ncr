...
exports.handler = async (event) => {
  ...
  const { ncr, method, history, examples, comments } = body;
...
Raised by: ${ncr.raised_by || 'Unknown'}`.trim();
  ...
  const commentsContext = (comments && comments.length)
    ? `\n\nDiscussion / comments on this NCR so far:\n${comments}`
    : '';

  // Few-shot examples from team-approved or team-corrected past RCAs.
  // These teach the model the team's preferred style, terminology, and
  // specificity for similar NCRs. Corrected examples carry more weight
  // — the team explicitly wrote the "right" answer.
  const examplesContext = (examples && examples.length)
    ? `\n\n<team_approved_examples>
Past RCAs from your Solidcor team for similar NCRs. Match THIS level of specificity,
naming, and detail. Prefer the team's wording over generic phrasing.

${examples.map((ex, i) => `--- Example ${i+1} ${ex.verdict==='corrected' ? "(team-corrected — the AI was wrong here, this is the team's better version)" : '(team-approved)'} ---
Issue: ${ex.issue || ''}
Category: ${ex.category || 'n/a'}  Department: ${ex.dept || 'n/a'}
Root cause: ${ex.root_cause || ''}
Corrective action: ${ex.corrective_action || ''}
Preventive action: ${ex.preventive_action || ''}${ex.team_notes ? `
Team's note: ${ex.team_notes}` : ''}`).join('\n\n')}
</team_approved_examples>`
    : '';
...
${ncrContext}${patternContext}${commentsContext}
